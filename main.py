...
# --- ⚙️ CONFIGURATION ---
PAY_DATA = {
    "KPay": {"Number": "09775394979", "Name": "Thansin Kyaw"},
    "Wave": {"Number": "09775394979", "Name": "Thansin Kyaw"},
    "AYAPay": {"Number": "09775394979", "Name": "Thansin Kyaw"},
...
# --- 💎 DIAMOND LIST DATA (သင့်ဆီက ပေးထားတဲ့ Data အသစ်ကို ထည့်သွင်းထားသည်) ---
GAMES_DATA = [
    {
        "name": "Normal Server (🇲🇲)",
...
<style>
    ...
    #home-sec { 
        ...
        background-size: cover; background-position: center; border-radius: 20px; padding: 20px 10px;
        min-height: 400px;
    }

    .game-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; }
    .game-card { background: rgba(30, 41, 59, 0.7); backdrop-filter: blur(5px); border: 1px solid #334155; border-radius: 15px; padding: 20px 10px; text-align: center; }
    .game-card img { width:50px; height:35px; border-radius:4px; margin-bottom:8px; }
    
    .cat-tabs-container { display:flex; gap:8px; overflow-x:auto; padding: 10px 0; scrollbar-width: none; }
    ...
    .pkg-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px; max-height: 400px; overflow-y: auto; padding: 5px; }
    ...
    .pay-btn { padding: 8px 15px; border-radius: 8px; border: 1px solid #334155; background: #1e293b; color: white; font-size: 12px; cursor: pointer; }
    .pay-btn.active { border-color: #fbbf24; background: #fbbf24; color: black; font-weight: bold; }
    
    .pay-card { background:rgba(30, 41, 59, 0.95); border-radius:15px; padding:15px; text-align:center; border:1px solid #fbbf24; margin-bottom: 15px; }
    ...
    .nav-btn { flex:1; text-align:center; color:#94a3b8; font-size:12px; text-decoration: none; }
</style>
</head><body>
...
<div id="order-sec" style="display:none;">
    ...
    <div class="pay-selector">
        <div class="pay-btn active" onclick="setPay('KPay', this)">KPay</div>
        <div class="pay-btn" onclick="setPay('Wave', this)">Wave</div>
        <div class="pay-btn" onclick="setPay('AYAPay', this)">AYAPay</div>
    </div>

    <div class="pay-card">
        <b id="pay-num" style="color:#fbbf24;font-size:18px;"></b>
    ...
    <form action="/order" method="post" enctype="multipart/form-data">
        ...
        <input type="file" name="photo" required accept="image/*" style="margin-top: 10px; border: none; background: none; padding: 0;">
        <button type="submit" class="buy-btn">CONFIRM & BUY</button>
    </form>
</div>

<div class="nav-bar">
    <a href="#" class="nav-btn" onclick="goHome()"><i class="fas fa-home"></i><br>Home</a>
...
function selectGame(name) {
    ...
    // ခလုတ်များကို Normal Dia, Weekly Pass, 2X Dia, Bundle Pack အစီအစဉ်အတိုင်း စီပေးခြင်း
...
function setPay(type, el) {
    if(el){document.querySelectorAll('.pay-btn').forEach(b=>b.classList.remove('active')); el.classList.add('active');}
...
function goHome(){ document.getElementById('order-sec').style.display='none'; document.getElementById('home-sec').style.display='block'; }
init();
</script></body></html>
'''
...
def order():
    ...
    return "<html><body style='background:#0f172a;color:white;text-align:center;padding:100px;'><h2>Order Success! ✅</h2><p>ခဏအကြာတွင် Home သို့ ပြန်သွားပါမည်။</p><script>setTimeout(()=>location.href='/', 2500);</script></body></html>"
