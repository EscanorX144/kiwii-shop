...
<style>
    ...
    .game-card { background: #1e293b; border: 1px solid #334155; border-radius: 15px; padding: 15px; text-align: center; }
    .game-card img { width: 60px; height: 60px; border-radius: 12px; }
...
<body>
    ...
    <div class="nav-bar">
        ...
        <div class="nav-btn" onclick="showHistory()">📜 History</div>
        <a href="{{cs}}" class="nav-btn">💬 CS</a>
    </div>
    ...
    function renderPkgs(sName, cat, el) {
        ...
        document.getElementById('pkg-list').innerHTML = data[sName].cats[cat].map(p => `
...
<!DOCTYPE html><html><body style="background:#0f172a;color:white;font-family:sans-serif;padding:20px;">
    ...
    <div style="background:#1e293b;padding:15px;border-radius:10px;margin-bottom:10px;">
        ...
        {{o.pkg}} | {{o.amt}} Ks<br>
        <a href="/update/{{o.id}}/Success" style="color:#10b981;">Success</a> | 
        <a href="/update/{{o.id}}/Cancel" style="color:#ef4444;">Cancel</a>
    </div>
    {% endfor %}
</body></html>
'''
