...
<style>
    ...
    /* Server Section background set to hero.webp */
    .game-grid { 
        display:grid; grid-template-columns:1fr 1fr; gap:15px; padding:20px; 
        background: url('/static/hero.webp') no-repeat center center; 
    ...
    /* Payment Icons from static folder */
    ...
    .copy-btn { background:#fbbf24; border:none; border-radius:5px; padding:6px 12px; font-weight:bold; cursor:pointer; margin-left:10px; }
    
    .glow-box { 
        background:#1e293b; padding:15px; border-radius:12px; border:2px solid #fbbf24; 
        box-shadow: 0 0 15px #fbbf24; text-align:center; margin:18px 0; font-weight:bold; color:#fbbf24;
    }
    ...
    .cat-tab { padding:10px 18px; background:#1e293b; border-radius:8px; border:1px solid #334155; cursor:pointer; white-space:nowrap; }
    .cat-tab.active { background:#fbbf24; color:black; font-weight:bold; }

    .history-card { background:#1e293b; padding:15px; margin-bottom:12px; border-radius:12px; border-left:6px solid #fbbf24; }
    .rank-item { display:flex; align-items:center; background:#1e293b; padding:12px; margin-bottom:10px; border-radius:10px; border:1px solid #334155; }
...
<div id="main-container">
    ...
    <div id="o-sec" style="display:none; padding:15px;">
        ...
        <div style="background:#1e293b; padding:20px; border-radius:15px; border:1px solid #334155;">
            <div class="pay-icons">
                <img src="/static/kpay.jpg"> 
                <img src="/static/wave.jpg"> 
                <img src="/static/ayapay.jpg">
            </div>
            <div style="text-align:center;">
                <b style="color:#fbbf24; font-size:22px;">09775394979</b>
                <button class="copy-btn" onclick="copyNum()">COPY</button><br>
                <small style="color:#94a3b8;">Name: Thansin Kyaw</small>
            </div>
        </div>
        ...
        <form id="orderForm" onsubmit="handleOrder(event)">
            ...
            <p style="font-size:13px;color:#94a3b8;margin:15px 0 5px 0;">Screenshot Upload:</p>
            <input type="file" id="photo" required accept="image/*" style="border:none; padding:0;">
...
function selG(id) {
    ...
    document.getElementById('hist-sec').style.display='none'; document.getElementById('top-sec').style.display='none';
...
function renderP(id, cat, el) {
    ...
    document.getElementById('p-list').innerHTML = pkgs.map(p=>`
        <div class="pkg-card" onclick="selP(this, '${p.d}', '${p.p}')">
            <span style="font-size:15px;">${p.d}</span><br><b style="color:#fbbf24; font-size:17px;">${p.p} Ks</b>
        </div>`).join('');
}
...
async function showH() {
    ...
    document.getElementById('hist-list').innerHTML = data.map(o => `
        <div class="history-card">
            <div style="display:flex; justify-content:space-between;"><b>${o.pkg}</b><b>${o.price} Ks</b></div>
            <small>${o.server} | ID: ${o.uid}(${o.zone})</small><br>
            <div style="display:flex; justify-content:space-between; margin-top:5px;">
                <small>${o.date}</small><b>${o.status}</b>
            </div>
...
async function showTop() {
    ...
    document.getElementById('top-list').innerHTML = data.map((u, i) => `
        <div class="rank-item">
            ...
            <div style="flex:1;"><b>ID: ${u._id}</b></div>
            <div style="color:#fbbf24;">${u.count} Orders</div>
        </div>`).join('') || "No Data Available.";
}
...
def order():
    try:
        uid, zid, server, pkg, price = request.form.get('uid'), request.form.get('zid'), request.form.get('server'), request.form.get('pkg'), request.form.get('price')
        photo = request.files.get('photo')
        oid = orders_col.insert_one({"uid": uid, "zone": zid, "server": server, "pkg": pkg, "price": price, "status": "Pending", "date": datetime.now(timezone(timedelta(hours=6, minutes=30))).strftime("%d/%m/%Y %I:%M %p")}).inserted_id
...
@app.route('/api/top10')
def get_top10():
    pipeline = [{"$group": {"_id": "$uid", "count": {"$sum": 1}}}, {"$sort": {"count": -1}}, {"$limit": 10}]
    top_users = list(orders_col.aggregate(pipeline))
